# Data documentation

> Главный регламент: [PROJECT_GUIDE.md](../00-project/PROJECT_GUIDE.md).

Раздел объединяет source snapshots, reference data, import, quality, baseline и publication evidence.

- [Immutable Source Snapshot](../immutable-source-snapshot-v1.md)
- [Reference Data Import](../reference-data-import-v1.md)
- [Legacy Import Pipeline](../legacy-import-pipeline-v1.md)
- [Product Import](../product-import-v1.md)
- [Catalog Quality Model](../catalog-quality-model-v1.md)
- [Catalog Baseline](../reports/catalog-baseline-v1.md)
- [Product Publication Foundation v1](./product-publication-foundation-v1.md)

Immutable source не редактируется. Supabase — operational Source of Truth; static fallback не является каноном.
