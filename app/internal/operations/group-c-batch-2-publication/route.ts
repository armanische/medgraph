...
import {
  GROUP_C_BATCH_2_PUBLICATION_MANIFEST_SHA256,
  validateGroupCBatch2PublicationOperationRequest,
} from "@/lib/operations/group-c-batch-2-publication-manifest";
import {
  executeProductionGroupCBatch2Publication,
  GroupCBatch2PublicationRunnerError,
} from "@/lib/operations/group-c-batch-2-publication-runner";
...
export async function POST(request: NextRequest) {
  ...
  if (!validateGroupCBatch2PublicationOperationRequest(body)) {
    return safeJson({ status: "blocked", code: "invalid_operation_manifest" }, 400, auth);
  }

  try {
    const result = await executeProductionGroupCBatch2Publication();
    return safeJson({
    ...
    const code = error instanceof GroupCBatch2PublicationRunnerError
      ? error.code
      : "operation_failed";
