...
import {
  ...
  validateIdn03RevisionOperationRequest,
} from "@/lib/operations/idn-03-revision-manifest";
import {
  executeProductionIdn03RevisionCreation,
  Idn03RevisionRunnerError,
} from "@/lib/operations/idn-03-revision-runner";
...
export async function POST(request: NextRequest) {
  ...
  if (!validateIdn03RevisionOperationRequest(body)) {
    return safeJson({ status: "blocked", code: "invalid_operation_manifest" }, 400, auth);
  }

  try {
    const result = await executeProductionIdn03RevisionCreation();
    return safeJson({
    ...
    const code = error instanceof Idn03RevisionRunnerError
      ? error.code
      : "operation_failed";
