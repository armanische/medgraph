...
> Historical MVP report. RFC-017 removed `publication:build`,
> `publication:audit`, and `publication:candidates`. References below describe
> the former pilot and are not current operating instructions.
...
Итого на момент пилота: 210 items, 0 approved, 0 eligible products. Текущий
readiness проверяется только в защищённом `/internal/reviewer`; удалённую CLI
команду использовать нельзя.
...
На момент пилота `Published products = 0`. Текущий публичный UI читает
Storefront Data Layer и не запускает автоматическую публикацию после Review.
