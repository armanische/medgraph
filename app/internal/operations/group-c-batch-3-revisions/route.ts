...
import {
  ...
  validateGroupCBatch3RevisionOperationRequest,
} from "@/lib/operations/group-c-batch-3-revision-manifest";
import {
  executeProductionGroupCBatch3RevisionCreation,
  GroupCBatch3RevisionRunnerError,
} from "@/lib/operations/group-c-batch-3-revision-runner";
...
export async function POST(request: NextRequest) {
  if (process.env.VERCEL_ENV !== "production") {
    return NextResponse.json(
      ...
      { status: 403, headers: { "Cache-Control": "private, no-store, max-age=0", "Referrer-Policy": "no-referrer", "X-Robots-Tag": "noindex, nofollow" } },
    );
  }
  ...
  if (!validateGroupCBatch3RevisionOperationRequest(body)) {
    return safeJson({ status: "blocked", code: "invalid_operation_manifest" }, 400, auth);
  }

  try {
    const result = await executeProductionGroupCBatch3RevisionCreation();
    return safeJson({
    ...
    const code = error instanceof GroupCBatch3RevisionRunnerError
      ? error.code
      : "operation_failed";
