...
import {
  calculateGroupCBatch3PublicationManifestSha256,
  GROUP_C_BATCH_3_PUBLICATION_MANIFEST,
  type GroupCBatch3PublicationManifestEntry,
} from "./group-c-batch-3-publication-manifest";
...
const EXCLUDED_PRODUCT_IDS = new Set([
  "24ac72fc-5c64-4f4e-9f92-cd4eca58e426", // IDN-03
  "db6da513-24dc-45e3-8e18-c6033825adce", // HUGER FB-53A
  "5c41e1d8-6311-4a63-bb99-41b8ae17d8a1", // PRIMEDIC Defi-B
...
export type GroupCBatch3PublicationResult = Readonly<{
...
export class GroupCBatch3PublicationRunnerError extends Error {
  readonly code: string;

  constructor(code: string) {
    super("Group C Batch 3 publication operation failed closed.");
    this.name = "GroupCBatch3PublicationRunnerError";
    this.code = code;
  }
}

function fail(code: string): never {
  throw new GroupCBatch3PublicationRunnerError(code);
}
...
function assertManifest() {
  const entries = GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries;
  ...
  if (
    calculateGroupCBatch3PublicationManifestSha256()
      !== GROUP_C_BATCH_3_PUBLICATION_MANIFEST.manifestSha256
    ||
    GROUP_C_BATCH_3_PUBLICATION_MANIFEST.productCount !== 7
    || productIds.size !== 7
    || sourceUids.size !== 7
    || revisionIds.size !== 7
    || decisionIds.size !== 7
    || reviewItemIds.size !== 7
...
function assertCatalogProduct(
  entry: GroupCBatch3PublicationManifestEntry,
...
async function approveExactly(
  ...
  const approvalIdempotencyKey = `group-c-batch-3-approval-${entry.revisionId}`;
...
async function publishExactly(
  ...
  const invoke = () => publishProduct({
    candidateRevisionId: entry.revisionId,
    idempotencyKey: `group-c-batch-3-publish-${entry.productId}`,
  }, client);
  let result;
  try {
    result = await invoke();
...
function finalResult(
  status: GroupCBatch3PublicationResult["status"],
  approvals: readonly ApprovalEvidence[],
  publications: readonly PublicationEvidence[],
): GroupCBatch3PublicationResult {
  return {
    status,
    operationKey: GROUP_C_BATCH_3_PUBLICATION_MANIFEST.operationKey,
    manifestSha256: GROUP_C_BATCH_3_PUBLICATION_MANIFEST.manifestSha256,
    approvals,
    publications,
    totals: {
      products: 79,
...
export async function executeProductionGroupCBatch3Publication(): Promise<GroupCBatch3PublicationResult> {
  assertManifest();
  ...
  const [inventory, projection, ...productValues] = await Promise.all([
    ...
    ...GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries.map(({ productId }) =>
  ...
  if (projection.products.length !== 63 && projection.products.length !== 70) {
    fail("published_scope_drift");
  }
  const alreadyCompleted = projection.products.length === 70;
  const products = GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries.map((entry, index) =>
  ...
  for (const entry of GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries) {
    approvalResults.push(await approveExactly(client, entry));
  }
  const approvalEvidence = approvalResults.map((result, index) => ({
    revisionId: GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries[index].revisionId,
    approvalId: result.approvalId,
  }));
  if (new Set(approvalEvidence.map(({ approvalId }) => approvalId)).size !== 7) {
    fail("approval_scope_invalid");
  }
  for (let index = 0; index < GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries.length; index += 1) {
    const confirmation = await approveExactly(
      client,
      GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries[index],
    );
  ...
  const publicationEvidence = publicationResults.map((result, index) => ({
    productId: GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries[index].productId,
    revisionId: GROUP_C_BATCH_3_PUBLICATION_MANIFEST.entries[index].revisionId,
    publicationBatchId: result.publicationBatchId,
    publicationVersion: result.publicationVersion,
    slug: products[index].slug,
  }));
  if (new Set(publicationEvidence.map(({ publicationBatchId }) => publicationBatchId)).size !== 7) {
  ...
  if (
    finalProjection.products.length !== 70
