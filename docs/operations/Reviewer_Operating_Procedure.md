...
5. Выполнить `npm run publication:candidates` и убедиться, что выбран один из пяти явно разрешённых продуктов.
...
5. Проверить product JSON в `data/public/products`, внутренний manifest и публичную карточку. Public JSON не должен содержать review/evidence/document-version IDs, artifact paths, comments, notes или SHA hashes.
