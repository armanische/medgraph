...
import {
  GROUP_C_BATCH_1_PUBLICATION_MANIFEST_SHA256,
  validateGroupCBatch1PublicationOperationRequest,
} from "@/lib/operations/group-c-batch-1-publication-manifest";
import {
  executeProductionGroupCBatch1Publication,
  GroupCBatch1PublicationRunnerError,
} from "@/lib/operations/group-c-batch-1-publication-runner";
...
export async function POST(request: NextRequest) {
  ...
  if (!validateGroupCBatch1PublicationOperationRequest(body)) {
    return safeJson({ status: "blocked", code: "invalid_operation_manifest" }, 400, auth);
  }

  try {
    const result = await executeProductionGroupCBatch1Publication();
    return safeJson({
    ...
    const code = error instanceof GroupCBatch1PublicationRunnerError
      ? error.code
      : "operation_failed";
