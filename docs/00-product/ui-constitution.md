...
### Principle 11A — Environment-local identity

Внутренние UUID являются environment-local identities и ЗАПРЕЩЕНЫ как бизнес-
идентичность для сопоставления между staging и Production.

Сущности между средами сопоставляются только по утверждённым стабильным или
natural keys, включая source UID, slug и source checksum. Перенос UUID из одной
среды в другую не является доказательством identity и не допускается в
publication или import workflow.
...
### Principle 16 — Single Operational Surface

Во время Launch Execution ЗАПРЕЩЕНО создавать новый административный раздел, если существующий внутренний интерфейс можно безопасно расширить.

- Предпочтение отдаётся развитию одного утверждённого Backoffice.
- Legacy internal tools не расширяются и не получают новые workflow.
- Новый внутренний workflow интегрируется в основной административный интерфейс при сохранении его Auth/RBAC и security boundary.
- Исключение допускается только через отдельное архитектурное решение с доказанной невозможностью безопасного расширения существующего Backoffice.
...
- [Launch Roadmap](../roadmap/Real_Data_Roadmap.md)
