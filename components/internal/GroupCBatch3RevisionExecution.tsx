...
export default function GroupCBatch3RevisionExecution() {
  ...
  async function execute() {
    ...
    try {
      const response = await fetch("/internal/operations/group-c-batch-3-revisions", {
        method: "POST",
      ...
      if (
        ...
        setState({
          ...
          message: result.status === "completed"
            ? "Семь immutable revisions созданы и replay-проверены."
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-200 bg-white p-6">
      <button
        ...
        type="button"
      >
        {state.status === "pending" ? "Проверка и создание…" : "Создать exact 7 revisions"}
      </button>
