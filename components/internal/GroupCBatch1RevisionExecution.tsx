...
const OPERATION_KEY = "group-c-batch-1-revision-creation-v1";
...
export default function GroupCBatch1RevisionExecution() {
  ...
  async function execute() {
    ...
    try {
      const response = await fetch("/internal/operations/group-c-batch-1-revisions", {
        method: "POST",
      ...
      if (
        ...
        setState({
          ...
          message: result.status === "completed"
            ? "Восемь immutable revisions созданы и replay-проверены."
            : "Операция уже была завершена; duplicate writes не созданы.",
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-200 bg-white p-6">
      <button
        ...
        type="button"
      >
        {state.status === "pending" ? "Проверка и создание…" : "Создать exact 8 revisions"}
      </button>
      ...
      {finished ? (
        <>
          <dl className="mt-4 grid gap-2 sm:grid-cols-2">
            ...
            <div><dt>Durable revisions</dt><dd>{state.revisionCount}</dd></div>
          </dl>
