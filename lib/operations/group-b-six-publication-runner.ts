...
import {
  GROUP_B_SIX_MANIFEST,
  type GroupBSixManifestEntry,
} from "./group-b-six-publication-manifest";
...
export type GroupBSixPublicationResult = Readonly<{
...
export class GroupBSixPublicationRunnerError extends Error {
  readonly code: string;

  constructor(code: string) {
    super("Group B six publication operation failed closed.");
    this.name = "GroupBSixPublicationRunnerError";
    this.code = code;
  }
}

function fail(code: string): never {
  throw new GroupBSixPublicationRunnerError(code);
}
...
function assertManifest() {
  const entries = GROUP_B_SIX_MANIFEST.entries;
  ...
  if (
    GROUP_B_SIX_MANIFEST.productCount !== 6
    || productIds.size !== 6
    || revisionIds.size !== 6
    || decisionIds.size !== 6
    || reviewItemIds.size !== 6
    || productIds.has("e7a54ec6-986d-422a-aca8-862d4d00a421")
    || entries.some((entry) =>
...
async function approveExactly(
  ...
  // The approved RPC is idempotent on candidate revision identity. The
  // deterministic operational key is group-b-six-approval-<revision-id>.
  const approvalIdempotencyKey = `group-b-six-approval-${entry.revisionId}`;
...
async function publishExactly(
  ...
  const invoke = () => publishProduct({
    candidateRevisionId: entry.revisionId,
    idempotencyKey: `group-b-six-publish-${entry.productId}`,
  }, client);
  let result;
  try {
    result = await invoke();
...
function finalResult(
  status: GroupBSixPublicationResult["status"],
  approvals: readonly ApprovalEvidence[],
  publications: readonly PublicationEvidence[],
): GroupBSixPublicationResult {
  return {
    status,
    operationKey: GROUP_B_SIX_MANIFEST.operationKey,
    manifestSha256: GROUP_B_SIX_MANIFEST.waveSha256,
    approvals,
    publications,
    totals: {
      products: 79,
...
export async function executeProductionGroupBSixPublication(): Promise<GroupBSixPublicationResult> {
  assertManifest();
  ...
  const [inventory, projection, ...productValues] = await Promise.all([
    ...
    ...GROUP_B_SIX_MANIFEST.entries.map(({ productId }) =>
  ...
  if (projection.products.length !== 36 && projection.products.length !== 42) {
    fail("published_scope_drift");
  }
  const alreadyCompleted = projection.products.length === 42;
  const products = GROUP_B_SIX_MANIFEST.entries.map((entry, index) =>
  ...
  for (const entry of GROUP_B_SIX_MANIFEST.entries) {
    approvalResults.push(await approveExactly(client, entry));
  }
  const approvalEvidence = approvalResults.map((result, index) => ({
    revisionId: GROUP_B_SIX_MANIFEST.entries[index].revisionId,
    approvalId: result.approvalId,
  }));
  const approvalIds = new Set(approvalEvidence.map(({ approvalId }) => approvalId));
  if (approvalIds.size !== 6) fail("approval_scope_invalid");

  for (let index = 0; index < GROUP_B_SIX_MANIFEST.entries.length; index += 1) {
    const confirmation = await approveExactly(client, GROUP_B_SIX_MANIFEST.entries[index]);
  ...
  const publicationEvidence = publicationResults.map((result, index) => ({
    productId: GROUP_B_SIX_MANIFEST.entries[index].productId,
    revisionId: GROUP_B_SIX_MANIFEST.entries[index].revisionId,
  ...
  if (publicationIds.size !== 6) fail("publication_scope_invalid");

  for (let index = 0; index < GROUP_B_SIX_MANIFEST.entries.length; index += 1) {
    const confirmation = await publishExactly(client, GROUP_B_SIX_MANIFEST.entries[index]);
    if (
  ...
  if (
    finalProjection.products.length !== 42
