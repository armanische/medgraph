...
const OPERATION_KEY = "group-c-batch-1-publication-v1";
...
export default function GroupCBatch1PublicationExecution() {
  ...
  async function execute() {
    ...
    try {
      const response = await fetch("/internal/operations/group-c-batch-1-publication", {
        method: "POST",
      ...
      if (
        ...
        && result.approvals?.length === 8
        && result.publications?.length === 8
        && result.totals?.published === 50
      ) {
        setState({
          status: result.status,
          message: result.status === "completed"
            ? "Approval 8/8 и Publication 8/8 завершены и replay-проверены."
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-200 bg-white p-6">
      <button
        ...
        type="button"
      >
        {state.status === "pending" ? "Проверка и публикация…" : "Опубликовать exact 8 Products"}
      </button>
