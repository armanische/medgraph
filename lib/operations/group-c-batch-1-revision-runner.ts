...
import {
  calculateGroupCBatch1RevisionManifestSha256,
  GROUP_C_BATCH_1_REVISION_COMPLETION_EVIDENCE,
  GROUP_C_BATCH_1_REVISION_MANIFEST,
  GROUP_C_BATCH_1_REVISION_MANIFEST_SHA256,
  type GroupCBatch1RevisionManifestEntry,
} from "./group-c-batch-1-revision-manifest";
...
const EXCLUDED_PRODUCTS = Object.freeze([
  ...
  { productId: "8bee3a8e-97a7-420a-aa9f-2f082136060d", sourceUid: "694791065122" },
...
export type GroupCBatch1RevisionResult = Readonly<{
  ...
  totals: Readonly<{
    ...
    decisions: 42;
    approvals: 42;
    publicationBatches: 42;
    pendingReviewItems: 8;
  }>;
  projectionVersion: 44;
}>;

export class GroupCBatch1RevisionRunnerError extends Error {
  readonly code: string;

  constructor(code: string) {
    super("Group C Batch 1 revision operation failed closed.");
    this.name = "GroupCBatch1RevisionRunnerError";
    this.code = code;
  }
}

function fail(code: string): never {
  throw new GroupCBatch1RevisionRunnerError(code);
}
...
function assertManifest() {
  const entries = GROUP_C_BATCH_1_REVISION_MANIFEST.entries;
  ...
  if (
    calculateGroupCBatch1RevisionManifestSha256()
      !== GROUP_C_BATCH_1_REVISION_MANIFEST_SHA256
    || GROUP_C_BATCH_1_REVISION_MANIFEST.productCount !== 8
    || entries.length !== 8
    || productIds.size !== 8
    || sourceUids.size !== 8
    || entries.some((entry) =>
...
function assertCatalogProduct(
  entry: GroupCBatch1RevisionManifestEntry,
...
async function preflight(client: SupabaseServerClient) {
  ...
  if (projection.products.length !== 42) fail("published_projection_drift");
  ...
  for (let read = 0; read < 10; read += 1) {
    rounds.push(await mapWithConcurrency(
      GROUP_C_BATCH_1_REVISION_MANIFEST.entries,
      3,
      ({ productId }) => readCatalogProduct(client, productId),
    ));
  }
  const states = GROUP_C_BATCH_1_REVISION_MANIFEST.entries.map((entry, index) => {
  ...
  if (created !== 0 && created !== 8) fail("mixed_revision_state");
  return created === 8;
}

function revisionIdempotencyKey(entry: GroupCBatch1RevisionManifestEntry) {
  return `group-c-batch-1-${entry.sourceUid}-revision-1`;
}
...
export async function executeProductionGroupCBatch1RevisionCreation(): Promise<
  GroupCBatch1RevisionResult
> {
  assertManifest();
  ...
  if (
    alreadyCreated
    && (
      GROUP_C_BATCH_1_REVISION_COMPLETION_EVIDENCE.length !== 8
      || new Set(GROUP_C_BATCH_1_REVISION_COMPLETION_EVIDENCE.map(({ productId }) => productId)).size !== 8
    )
  ) fail("preexisting_revision");

  // The approved maximum is three. A sequential write order minimizes the
  // failure surface while retaining per-Product idempotent recovery.
  const firstResults: Awaited<ReturnType<typeof createProductPublicationRevision>>[] = [];
  for (const entry of GROUP_C_BATCH_1_REVISION_MANIFEST.entries) {
    firstResults.push(await createExactly(client, entry));
  }
  const created = firstResults.filter(({ idempotent }) => !idempotent).length;
  if (created !== (alreadyCreated ? 0 : 8)) fail("partial_revision_creation");

  if (alreadyCreated) {
    firstResults.forEach((result, index) => {
      const expected = GROUP_C_BATCH_1_REVISION_COMPLETION_EVIDENCE[index];
      if (
        !result.idempotent
  ...
  if (revisionIds.size !== 8 || reviewItemIds.size !== 8) fail("revision_binding_duplicate");
  ...
  afterProducts.forEach((product, index) => {
    assertCatalogProduct(GROUP_C_BATCH_1_REVISION_MANIFEST.entries[index], product, true);
  });
  ...
  if (afterProjection.products.length !== 42) fail("published_projection_drift");
  ...
  return {
    ...
    manifestSha256: GROUP_C_BATCH_1_REVISION_MANIFEST.manifestSha256,
    created,
    revisions: replayResults.map((result, index) =>
      evidenceFromResult(GROUP_C_BATCH_1_REVISION_MANIFEST.entries[index], result)),
    totals: {
      products: 79,
      published: 42,
      revisions: 50,
