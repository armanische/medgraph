...
test("catalog and manufacturer pages use Storefront services", async () => {
  ...
  assert.doesNotMatch(manufacturerPage, /getPublishedManufacturerProducts/u);
  assert.match(
    manufacturerPage,
    /manufacturerService\.getManufacturerBySlug\(slug\)/u,
  );
  assert.doesNotMatch(manufacturersPage, /getPublishedCatalog/u);
  assert.match(
    manufacturersPage,
    /manufacturerService\.getManufacturers\(\)/u,
  );
