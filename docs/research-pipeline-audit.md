...
| `/internal/review-queue` | `app/internal/review-queue/page.tsx` → `lib/review/human-workspace.ts` → canonical Human Review loader/store | `data/research/review/products`, integrity, extraction, and append-only decisions | Read-only projection of canonical Human Review state | Move the shared Human Review repository/store outside the Research namespace |
