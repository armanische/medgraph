...
import {
  GROUP_C_BATCH_1_PUBLICATION_MANIFEST,
  type GroupCBatch1PublicationManifestEntry,
} from "./group-c-batch-1-publication-manifest";
...
const EXCLUDED_PRODUCT_IDS = new Set([
  ...
  "7efe1eb2-6551-4f0b-9310-c898cbcfdf7a", // combined UNIKOS
  "8bee3a8e-97a7-420a-aa9f-2f082136060d", // UNIKOS-01
...
export type GroupCBatch1PublicationResult = Readonly<{
...
export class GroupCBatch1PublicationRunnerError extends Error {
  readonly code: string;

  constructor(code: string) {
    super("Group C Batch 1 publication operation failed closed.");
    this.name = "GroupCBatch1PublicationRunnerError";
    this.code = code;
  }
}

function fail(code: string): never {
  throw new GroupCBatch1PublicationRunnerError(code);
}
...
function assertManifest() {
  const entries = GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries;
  ...
  const reviewTimestamp = /^2026-08-01T\d{2}:\d{2}:\d{2}\.\d{6}Z$/u;
  if (
    GROUP_C_BATCH_1_PUBLICATION_MANIFEST.productCount !== 8
    || productIds.size !== 8
    || sourceUids.size !== 8
    || revisionIds.size !== 8
    || decisionIds.size !== 8
...
function assertCatalogProduct(
  entry: GroupCBatch1PublicationManifestEntry,
...
async function approveExactly(
  ...
  const approvalIdempotencyKey = `group-c-batch-1-approval-${entry.revisionId}`;
...
async function publishExactly(
  ...
  const invoke = () => publishProduct({
    candidateRevisionId: entry.revisionId,
    idempotencyKey: `group-c-batch-1-publish-${entry.productId}`,
  }, client);
  let result;
  try {
    result = await invoke();
...
function finalResult(
  status: GroupCBatch1PublicationResult["status"],
  approvals: readonly ApprovalEvidence[],
  publications: readonly PublicationEvidence[],
): GroupCBatch1PublicationResult {
  return {
    status,
    operationKey: GROUP_C_BATCH_1_PUBLICATION_MANIFEST.operationKey,
    manifestSha256: GROUP_C_BATCH_1_PUBLICATION_MANIFEST.manifestSha256,
    approvals,
    publications,
    totals: {
      products: 79,
...
export async function executeProductionGroupCBatch1Publication(): Promise<GroupCBatch1PublicationResult> {
  assertManifest();
  ...
  const [inventory, projection, ...productValues] = await Promise.all([
    ...
    ...GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries.map(({ productId }) =>
  ...
  if (projection.products.length !== 42 && projection.products.length !== 50) {
    fail("published_scope_drift");
  }
  const alreadyCompleted = projection.products.length === 50;
  const products = GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries.map((entry, index) =>
  ...
  for (const entry of GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries) {
    approvalResults.push(await approveExactly(client, entry));
  }
  const approvalEvidence = approvalResults.map((result, index) => ({
    revisionId: GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries[index].revisionId,
    approvalId: result.approvalId,
  }));
  if (new Set(approvalEvidence.map(({ approvalId }) => approvalId)).size !== 8) {
    fail("approval_scope_invalid");
  }
  for (let index = 0; index < GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries.length; index += 1) {
    const confirmation = await approveExactly(
      client,
      GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries[index],
    );
  ...
  const publicationEvidence = publicationResults.map((result, index) => ({
    productId: GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries[index].productId,
    revisionId: GROUP_C_BATCH_1_PUBLICATION_MANIFEST.entries[index].revisionId,
    publicationBatchId: result.publicationBatchId,
    publicationVersion: result.publicationVersion,
    slug: products[index].slug,
  }));
  if (new Set(publicationEvidence.map(({ publicationBatchId }) => publicationBatchId)).size !== 8) {
  ...
  if (
    finalProjection.products.length !== 50
